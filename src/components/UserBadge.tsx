...
export default function UserBadge({ user, isOwner }: UserBadgeProps) {
    return (
        <div className="relative group">
            <div className="flex items-center gap-3 glass-card px-4 py-2 rounded-full border-white/10 group-hover:border-white/30 transition-all duration-300 cursor-pointer">
                ...
                {user.image ? (
                    <img src={user.image} alt="" className="w-8 h-8 rounded-full border border-white/20 group-hover:border-white/50 transition-colors" />
                ) : (
