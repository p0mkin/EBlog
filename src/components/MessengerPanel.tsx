...
export default function MessengerPanel({ chatId, currentUserId, isAdmin, participantName, onClose }: { chatId: string; currentUserId?: string; isAdmin: boolean; participantName?: string; onClose?: () => void }) {
    ...
    useEffect(() => {
        fetchMessages();
        const intv = setInterval(fetchMessages, 3000); // Fast polling for chat
    ...
    return (
        <div className="flex flex-col h-full bg-[#111] overflow-hidden">
            ...
            <div className="p-4 border-b border-white/10 flex items-center justify-between shrink-0 bg-black/40 backdrop-blur-md">
                ...
                {isAdmin && (
                    <span className="text-xs px-2 py-0.5 rounded uppercase tracking-widest font-bold bg-white/10 text-white">Chat</span>
                )}
            </div>

            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-black/40">
                {loading ? (
                    <div className="flex items-center justify-center h-full">
                        <span className="w-8 h-8 rounded-full border-4 border-white/10 border-t-white animate-spin"></span>
                    </div>
                ) : messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-zinc-500 space-y-3">
                        <svg className="w-16 h-16 opacity-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
                        <p className="text-sm font-mono tracking-wide">Say hello! End-to-end securely stored.</p>
                    </div>
                ) : (
                            ...
                            return (
                                <div key={msg.id} className={`flex w-full ${isMe ? "justify-end" : "justify-start"}`}>
                                    <div className={bubbleClass}>
                                        ...
                                        {msg.type === "image" && (
                                            <img src={`/api/photos/download?key=${msg.mediaKey}`} className="rounded-xl max-h-60 object-cover" alt="Image message" loading="lazy" />
                                        )}
            ...
            <div className="shrink-0 p-4 border-t border-white/10 flex items-center gap-3 bg-black/40">
                ...
                {text.trim() ? (
                    <button onClick={handleSendText} className="shrink-0 w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center shadow-lg hover:bg-indigo-500 hover:scale-105 transition active:scale-95">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="ml-0.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                    </button>
                ) : (
                    <button 
